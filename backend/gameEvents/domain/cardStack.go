...
var CardStack = []Card{
	*NewCard("You inherited", "You're mentioned in the testament of your aunt. You receive 100 $.", func(playerId string) {
		communication.FireEvent(communication.CARD_WITH_PAYOUT_DREW, NewCardWithPayoutDrewEvent(playerId, 100))
	}),
	*NewCard("Tax bill", "You received a bill for the federal taxes of 200 $", func(playerId string) {
		communication.FireEvent(communication.CARD_WITH_FEE_DREW, NewCardWithFeeDrewEvent("Bank", playerId, 200))
	}),
}
